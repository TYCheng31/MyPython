        ...
        if participation_data:
            ...
            if (user_id, contest_id) not in user_contest_scores:
                ...
                if task_id not in user_contest_scores[(user_id, contest_id)]["task_scores"]:
                    ...
                    if score > user_contest_scores[(user_id, contest_id)]["task_scores"][task_id]:
                        user_contest_scores[(user_id, contest_id)]["task_scores"][task_id] = score
                        user_contest_scores[(user_id, contest_id)]["timestamps"][task_id] = timestamp
                    elif score == user_contest_scores[(user_id, contest_id)]["task_scores"][task_id]:
                        # 如果分數相同，保留較新的 timestamp
                        if timestamp > user_contest_scores[(user_id, contest_id)]["timestamps"][task_id]:
                            user_contest_scores[(user_id, contest_id)]["timestamps"][task_id] = timestamp
...
for (user_id, contest_id), data in user_contest_scores.items():
    ...
    final_results.append({
        ...
        "timestamp": latest_timestamp.strftime("%Y-%m-%d %H:%M:%S")  # 格式化 timestamp 顯示
    })
...
with open("output.csv", "w", newline="") as csvfile:
    # 設定最終要寫入 CSV 的欄位名稱
    fieldnames = ["first_name", "contest_name", "contest_start", "timestamp", "total_score"]
